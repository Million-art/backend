...
export class MaterialController {
  ...
  @ApiQuery({ 
    ...
    example: 'createdAt',
    enum: ['createdAt', 'title', 'downloadCount', 'viewCount']
  })
  @ApiQuery({ 
    name: 'sortOrder', 
    required: false, 
  ...
  @Post(':id/download')
  @ApiOperation({ summary: 'Download a material (increments download count)' })
  ...
  async download(@Param('id') id: string) {
    ...
    const updatedMaterial = await this.updateMaterialUseCase.execute(id, {
      downloadCount: (material.downloadCount || 0) + 1
    });
    
    // Return download URL
    const downloadUrl = material.filePath || material.url;
    if (!downloadUrl) {
      throw new BadRequestException('No download URL available for this material');
    }
  ...
  @Post(':id/view')
  @ApiOperation({ summary: 'View a material (increments view count)' })
  @ApiParam({ name: 'id', description: 'Material ID' })
  ...
  async view(@Param('id') id: string) {
    ...
    await this.updateMaterialUseCase.execute(id, {
