...
export class QuizController {
  ...
  async submitAttempt(
    ...
    @Body() attemptData: { answers: Array<{ questionId: string; selectedAnswer: string | boolean }> }
  ) {
    const quiz = await this.getQuizUseCase.execute(quizId);
    
    // Calculate score
    let totalScore = 0;
    const answers = attemptData.answers.map(answer => {
    ...
    const percentage = Math.round((totalScore / quiz.questions.reduce((sum, q) => sum + q.points, 0)) * 100);
    
    // In a real app, you would save this to a database
    const attempt = {
      id: `attempt-${Date.now()}`,
      quizId,
      userId: 'user-1', // In a real app, get from JWT token
      score: totalScore,
      totalQuestions,
      percentage,
